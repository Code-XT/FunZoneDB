...
const BASE_Games = `https://api.rawg.io/api/games?key=${GAMES_API}`;
...
export const GamesSearch = async (term, type) => {
  const res = await fetch(
    `${BASE_Games}&search=${term}&search_precise=true&search_exact=true&platforms=${type.join(
      ","
    )}`
  );
  const data = await res.json();
...
export const GameDetail = async (id) => {
  const res = await fetch(`${BASE_MAL}&id=${id}`);
  const data = await res.json();
  return data;
};
...
// export const MovieDetail = async (term, type) => {
//   const res = await fetch(`${BASE_MAL}/${type}?q=${term}`);
//   const data = await res.json();
//   return data;
// };
